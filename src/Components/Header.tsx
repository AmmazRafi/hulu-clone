...
function Header() {
  ...
  return (  
    <div className='flex z-30 justify-between items-center p-4 px-10 absolute w-full
    bg-gradient-to-b from-[#1e2126] to-transparent'>
        ...
        <ul className='hidden md:flex gap-8'>
            {menu.map((item)=>(
                ...
                hover:text-white px-3 pb-2 py-1" ${item.id==1?"bg-gray-700 text-white":null} rounded-md
                transition-all duration-500 ease-in-out `}>{item.name}</li>
            ))}
        </ul>

      <div className='md:hidden'>
        <h2 className='text-white ml-[100px]  font-medum flex items-center gap-2 px-3 py-2 pb-2 
        ...
        <div className='flex gap-10'>
            ...
            <h2 className='px-[15px] text-[20px] text-gray-300
            border-[2px] border-white rounded-full '>Ammaz</h2>
        </div>
    </div>
  )
}
